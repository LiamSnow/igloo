...
pub fn make_test_tree(num_floes: usize, num_groups: usize, num_devices: usize) -> DeviceTree {
    ...
    let mut groups: Vec<Option<Group>> = (0..num_groups.max(50))
        .map(|i| {
            if i < num_groups {
                Some(Group {
                    id: GroupID::from_parts(i as u32, 0),
                    name: ROOM_NAMES
    ...
    let mut devices: Vec<Option<Device>> = Vec::with_capacity(num_devices.max(200));
    ...
    devices.resize_with(num_devices.max(200), || None);

    for device in devices.iter().flatten() {
        for gid in &device.groups {
            if let Some(Some(group)) = groups.get_mut(gid.index() as usize) {
                group.devices.insert(device.id);
            }
        }
    }
    groups.resize_with(num_groups.max(50), || None);
