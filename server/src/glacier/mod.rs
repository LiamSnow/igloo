...
use crate::glacier::query::{GlobalArea, GlobalQueryRequest, LocalArea, LocalQueryRequest};
...
#[derive(Debug)]
pub struct Floe {
    query_tx: mpsc::Sender<LocalQueryRequest>,
}
...
impl GlacierState {
    pub async fn dispatch_query(&self, req: GlobalQueryRequest) {
        ...
        match req.area {
            ...
            GlobalArea::Device(persistent_id) => {
            ...
            GlobalArea::Entity(persistent_id, entity_name) => {
                let Some(device) = self.devices.get(&persistent_id) else {
                    panic!("Invalid device ID {persistent_id}");
                };
