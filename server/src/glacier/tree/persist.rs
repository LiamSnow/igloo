...
impl DeviceTree {
    pub async fn load() -> Result<Self, TreePersistError> {
    ...
    async fn load_groups() -> Result<Vec<Option<Group>>, TreePersistError> {
        if !fs::try_exists(GROUPS_FILE).await? {
            return Ok(Vec::with_capacity(50));
        }

        let content = fs::read_to_string(GROUPS_FILE).await?;
    ...
    pub(super) async fn save_groups(&self) -> Result<(), TreePersistError> {
        let mut ini = Ini::new();
    ...
    pub(super) async fn save_devices(&self) -> Result<(), TreePersistError> {
        let mut ini = Ini::new();
